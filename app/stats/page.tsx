...
function getGapClass(value: number | null) {
  if (value === null || value === 0) return 'text-[var(--ink-soft)]'
...
function DistributionPanel({
  ...
  return (
    <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-5 h-full" data-testid={testId}>
      <div className="flex items-center justify-between gap-3 mb-4">
        <h3 className="text-base font-bold text-[var(--ink)]">{title}</h3>
        {items.length > 0 && (
          <span className="text-xs font-bold text-[var(--ink-soft)]">{formatPeople(total)}</span>
        )}
      </div>
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[9rem_minmax(0,1fr)] gap-5 items-center">
          <div className="relative w-36 h-36">
            ...
            >
              <circle cx="20" cy="20" r="15.915" fill="none" stroke="var(--line)" strokeWidth="6" />
              {segments.map((item) => (
                <circle
                  key={item.label}
                  cx="20"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-mono text-3xl font-bold text-[var(--ink)]">{total}</span>
              <span className="text-[10px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">people</span>
            </div>
          </div>
              ...
              <div key={item.label}>
                <div className="flex items-center justify-between gap-3">
                  <span className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    />
                    <span className="text-xs font-bold text-[var(--ink)] truncate" title={item.label}>
                      {item.label}
                    </span>
                  </span>
                  <span className="font-mono text-xs font-medium text-[var(--ink-soft)] flex-shrink-0">
                    {item.count} · {item.percent}%
                  </span>
                </div>
                <div className="h-1.5 rounded-full bg-[var(--paper)] mt-1.5 overflow-hidden">
                  <div
...
function ScoreSnapshot({ data }: { data: StatsResponse }) {
  ...
  return (
    <div
      className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-5 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-y-5 sm:divide-x sm:divide-[var(--line)] w-full"
      data-testid="score-snapshot"
    >
      {stats.map((stat) => (
        <div key={stat.label} className="sm:px-4 sm:first:pl-0">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">{stat.label}</p>
          <p className="font-mono text-xl font-bold text-[var(--ink)] mt-1">{stat.value}</p>
        </div>
      ))}
    </div>
  )
...
function PlaceInsightsPanel({
  ...
  return (
    <div className="space-y-3" data-testid="place-insights">
      <div>
        <h3 className="text-base font-bold text-[var(--ink)]">Your standing in {scopeLabel}</h3>
        <p className="text-xs text-[var(--ink-soft)] mt-1">How your score compares with nearby and similar test-takers</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {items.map((item) => (
          <div key={item.label} className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-4" data-testid={item.testId}>
            <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">{item.label}</p>
            <p
            ...
            <p
              className={`mt-1 truncate text-xs ${item.detailClass ?? 'text-[var(--ink-soft)]'}`}
              title={item.detail}
            >
              {item.detail}
            </p>
...
function RankedBarList({
  ...
  return (
    <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-5 h-full" data-testid={testId}>
      <div className="flex items-center justify-between gap-3 mb-4">
        <h3 className="text-base font-bold text-[var(--ink)]">{title}</h3>
        {items.length > 0 && <span className="font-mono text-xs font-medium text-[var(--ink-soft)]">{total}</span>}
      </div>
      {items.length === 0 ? (
        <p className="text-sm text-[var(--ink-soft)]">No data yet.</p>
      ) : (
        <ol className="space-y-3">
          {displayItems.map((item, index) => (
            <li key={item.label} className="flex items-center gap-3">
              <span className="w-4 flex-shrink-0 font-mono text-xs text-[var(--ink-soft)]">{index + 1}</span>
...
function AverageScoreChart({
  ...
  return (
    <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-3.5" data-testid={testId}>
      <div className="flex items-start justify-between gap-3 mb-2.5">
        <div>
          <h3 className="text-sm font-bold text-[var(--ink)] leading-snug">{title}</h3>
          <p className="text-[11px] text-[var(--ink-soft)] mt-0.5 leading-tight">{subtitle}</p>
        </div>
        <span className="font-mono text-[10px] font-semibold text-[var(--ink-soft)] bg-[var(--paper)] rounded-full px-2 py-0.5 whitespace-nowrap flex-shrink-0">
          out of 10
        </span>
      </div>
            ...
            <div key={item.label} className="py-1 first:pt-0 last:pb-0">
              <div className={`grid ${rowGridClass} items-baseline gap-2 mb-0.5`}>
                <div className="min-w-0">
                  <p className="text-[11px] font-bold text-[var(--ink)] leading-tight break-words">
                    {item.label}
                  </p>
                </div>
                <span className="text-left text-[10px] font-medium text-[var(--ink-soft)] whitespace-nowrap">
                  {formatPeople(item.count)}
                </span>
                <span aria-hidden="true" />
                <p className="text-right font-mono text-sm font-bold text-[var(--signal)] leading-none">
                  {item.averageScore}/10
                </p>
              </div>
              <div className="h-1 rounded-full bg-[var(--paper)] overflow-hidden">
                <div
                  className="h-full rounded-full bg-[var(--signal)] transition-all duration-500 ease-out"
...
function LocationComparisonChart({ items }: { items: LocationComparisonItem[] }) {
  ...
  return (
    <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-4 w-full" data-testid="local-global-comparison">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <h3 className="text-sm font-bold text-[var(--ink)]">Local vs global</h3>
          <p className="text-[11px] text-[var(--ink-soft)] mt-0.5">Average score for this domain by scope</p>
        </div>
        <span className="font-mono text-[10px] font-semibold text-[var(--ink-soft)] bg-[var(--paper)] rounded-full px-2 py-0.5 whitespace-nowrap">
          latest attempts
        </span>
      </div>
      {displayItems.length === 0 ? (
        <p className="text-sm text-[var(--ink-soft)]">No comparison data yet.</p>
      ) : (
        <div className="divide-y divide-[var(--line)]">
          {displayItems.map((item) => (
            <div key={`${item.scope}-${item.label}`} className="py-2 first:pt-0 last:pb-0">
              <div className="flex items-baseline justify-between gap-3 mb-1">
                <div className="min-w-0">
                  <p className="text-xs font-bold text-[var(--ink)] truncate">{item.label}</p>
                  <p className="grid grid-cols-[6.75rem_auto] gap-2 text-[11px] text-[var(--ink-soft)]">
                    <span>{item.scope}</span>
                    <span className="whitespace-nowrap">{formatPeople(item.count)}</span>
                  </p>
                </div>
                <span className="font-mono text-sm font-bold text-[var(--signal)] flex-shrink-0">
...
function ProgressPanel({ data }: { data: StatsResponse }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" data-testid="progress-panels">
      <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-4">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">Progress over time</p>
        <p className="font-mono text-2xl font-bold text-[var(--ink)] mt-2">
          {hasAttempt ? formatScore(progress.latestScore) : '-'}
        </p>
        <p className="text-xs text-[var(--ink-soft)] mt-2">
          Previous <span className="font-mono font-bold text-[var(--signal)]">{formatScore(progress.previousScore)}</span>
        </p>
        <p className="text-xs text-[var(--ink-soft)] mt-1">
          Change <span className={`font-mono text-sm font-bold ${changeClass}`}>{formatChange(progress.scoreChange)}</span>
        </p>
        <p className="text-xs text-[var(--ink-soft)] mt-1">
          Average <span className="font-mono font-bold text-[var(--signal)]">{formatScore(progress.consistency.averageScore)}</span>
        </p>
      </div>

      <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-4">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">Best vs latest</p>
        <p className="font-mono text-2xl font-bold text-[var(--ink)] mt-2">
          {formatScore(progress.bestScore)}
        </p>
        <p className="text-xs text-[var(--ink-soft)] mt-2">
          Latest <span className="font-mono text-sm font-bold text-[var(--signal)]">{formatScore(progress.latestScore)}</span>
        </p>
      </div>

      <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-4">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">Time efficiency</p>
        <p className="font-mono text-2xl font-bold text-[var(--ink)] mt-2">
          {formatDuration(progress.latestTimeSeconds)}
        </p>
        {!hasAttempt && <p className="text-xs text-[var(--ink-soft)] mt-2">No attempt yet</p>}
        <p className="text-xs text-[var(--ink-soft)] mt-1">
          Community avg <span className="font-mono font-bold text-[var(--signal)]">{formatDuration(data.averageTimeSeconds)}</span>
        </p>
      </div>

      <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-4">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">Consistency</p>
        <p className={`font-mono text-2xl font-bold mt-2 ${consistencyClass}`}>
          {progress.consistency.label}
        </p>
        <p className="text-xs text-[var(--ink-soft)] mt-2">{consistencyDetail}</p>
      </div>
    </div>
  )
}
...
function DomainStrengthRanking({ overview }: { overview: OverviewResponse | null }) {
  if (!overview) {
    return (
      <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-5 w-full" data-testid="domain-strength-ranking">
        <p className="text-[var(--ink-soft)] text-sm animate-pulse">Loading domain strengths...</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-5 w-full" data-testid="domain-strength-ranking">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <h3 className="text-base font-bold text-[var(--ink)]">Domain strength ranking</h3>
          <p className="text-xs text-[var(--ink-soft)] mt-1">Your latest score, best to weakest</p>
        </div>
      </div>
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between gap-3 mb-1">
              <span className="flex items-baseline gap-2 min-w-0">
                <span className="font-mono text-xs text-[var(--ink-soft)] w-4">{index + 1}</span>
                <span className="text-xs font-bold text-[var(--ink)] leading-snug break-words">{item.label}</span>
              </span>
              {item.score === null ? (
                <span className="w-44 text-right text-[11px] font-medium text-[var(--ink-soft)] flex-shrink-0">
                  Not attempted
                </span>
              ) : (
                <span className="grid w-44 grid-cols-2 gap-3 text-[11px] font-medium text-[var(--ink-soft)] flex-shrink-0">
                  <span>
                    Latest <span className="font-mono font-bold text-[var(--signal)]">{formatScore(item.score)}</span>
                  </span>
                  <span>
                    Best <span className="font-mono font-bold text-[var(--signal)]">{formatScore(item.best ?? item.score)}</span>
                  </span>
                </span>
              )}
            </div>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="text-lg font-bold text-[var(--ink)] pt-4 mt-1 border-t border-[var(--line)]">{children}</h2>
  )
}
        ...
        {tab === 'performance' && (
          <div className="space-y-4">
            {!loading && !error && data && data.totalUsers === 0 && (
              <p className="text-[var(--ink-soft)] text-sm" data-testid="no-attempts-yet">
            ...
            <div className="bg-[var(--surface)] rounded-lg border border-[var(--line)] p-5">
              {loading && <p className="text-[var(--ink-soft)] text-sm animate-pulse">Loading stats…</p>}
              {!loading && error && <p className="text-red-600 text-sm">{error}</p>}

              {!loading && !error && data && data.totalUsers < MIN_SAMPLE_SIZE && (
                <p className="text-[var(--ink-soft)] text-sm text-center py-10" data-testid="stats-empty">
              ...
              {!loading && !error && data && data.totalUsers >= MIN_SAMPLE_SIZE && (
                <div data-testid="stats-chart">
                  <div className="flex flex-wrap items-end justify-between gap-3 mb-5">
                    <div>
                      <h3 className="text-base font-bold text-[var(--ink)]">Score distribution</h3>
                      <p className="text-xs text-[var(--ink-soft)] mt-1">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      {data.averageScore !== null && (
                        <span
                          className="flex h-16 w-36 flex-col justify-center rounded-md border border-[var(--line)] bg-[var(--paper)] px-3 py-2"
                          data-testid="chart-average"
                        >
                          <span className="block text-[10px] font-semibold uppercase tracking-wide text-[var(--ink-soft)]">
                            Average
                          </span>
                          <span className="block font-mono text-sm font-bold text-[var(--signal)] leading-tight">
                      ...
                      {data.topScore !== null && (
                        ...
                        >
                          ...
                          <span className="block text-[10px] font-medium text-[var(--ink-soft)] leading-tight">
                  ...
                  <div className="grid grid-cols-[1rem_2rem_minmax(0,1fr)] gap-x-3">
                    ...
                    <div className="relative h-56 w-4">
                      <p className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-90 whitespace-nowrap text-xs text-[var(--ink-soft)]">
                        Test-takers (%)
                      </p>
                    </div>
                    <div className="flex flex-col justify-between h-56 font-mono text-[11px] text-[var(--ink-soft)] text-right">
                      {yAxisTicks.map((tick) => (
                    ...
                    <div className="flex-1 relative h-56">
                      <div className="absolute inset-0 flex flex-col justify-between">
                        {yAxisTicks.map((tick) => (
                          <div key={tick} className="border-t border-[var(--line)] w-full" />
                        ))}
                      </div>
                        ...
                        {data.histogram.map((count, score) => {
                          ...
                          return (
                            ...
                            >
                              {isYou && (
                                <span
                                  className="mb-1 w-6 h-6 rounded-full bg-[var(--signal)] text-white text-[10px] flex items-center justify-center font-bold flex-shrink-0"
                                  title="Your score"
                              ...
                              {count > 0 && (
                                <span className="font-mono text-[10px] font-semibold text-[var(--ink-soft)] mb-1">
                                  {count}
                                </span>
                              )}
                              <div
                                className={`w-full rounded-t-sm transition-all duration-500 ease-out ${isYou ? 'bg-[var(--signal)]' : 'bg-[var(--signal-soft)]'}`}
                                style={{
                  ...
                  <div className="grid grid-cols-[1rem_2rem_minmax(0,1fr)] gap-x-3 mt-2">
                    ...
                    <div className="flex gap-2">
                      {data.histogram.map((_, score) => (
                        <span key={score} className="flex-1 text-center font-mono text-[11px] text-[var(--ink-soft)]">
                          {score}
                        </span>
                      ))}
                    </div>
                  </div>
                  <p className="text-center text-xs text-[var(--ink-soft)] mt-2">Score (out of 10)</p>
                </div>
              )}
            </div>
